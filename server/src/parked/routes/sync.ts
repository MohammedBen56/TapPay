...
/** Two outcomes that can never be persisted to offline_intents (no trusted
 * tx_uuid, or the FK on receiver_id can't be satisfied) -- response-only, never
 * written to the DB, unlike the five OfflineIntentStatus values which are. */
type SyncResultStatus = OfflineIntentStatus | "FAILED_INVALID_SIGNATURE" | "FAILED_UNKNOWN_RECIPIENT" | "FAILED_SELF_PAYMENT";
...
export function registerSyncRoutes(app: FastifyInstance): void {
  app.post("/tx/sync", async (request, reply) => {
...
async function processOneIntent(
  ...
  const txUuid = bytesToUuid(iou.tx_uuid);
  ...
  if (!result.success) {
    await db.updateTable("offline_intents").set({ status: "FAILED_INSUFFICIENT" }).where("tx_uuid", "=", txUuid).execute();
    return { tx_uuid: txUuid, status: "FAILED_INSUFFICIENT" };
  }
