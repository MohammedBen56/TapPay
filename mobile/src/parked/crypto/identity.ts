...
import { cacheOwnCredential, getEnrolledIdentity, saveEnrolledIdentity } from '../db/offlineIntents';
import {
 ...
 * and cache the result locally; paymentFlow.ts's signAndQueueIou checks the
 * cached token's age client-side before allowing an offline send, and the
 * server independently re-verifies it at /tx/sync time -- this call itself
 * does no local caching.
 */
...
/**
 * Fetches THIS device's own server-signed credential and caches it locally
 * (M3 Milestone 3) -- structurally the live fetch half of fetchPeerCredential
 * above, but for one's own device_id, and persisted (mobile/src/db/
 * offlineIntents.ts's cacheOwnCredential) rather than just returned. Every
 * SessionHello this device signs (payments/bleTransport.ts) embeds the
 * cached result instead of requiring the PEER to fetch this device's
 * credential live -- the fix for BLE settlement previously requiring BOTH
 * parties to have live server reachability, not just the radio. Call this
 * opportunistically whenever online (right after enrollment is the best
 * first opportunity, mirroring fetchFreshnessToken's own doc comment) --
 * non-fatal if it fails; bleTransport.ts fails closed to the QR fallback if
 * no cached credential ends up available when a BLE attempt needs one.
 * Verifies before caching -- never persist unverified bytes, even though
 * they came from this device's own enrollment record.
 */
export async function fetchAndCacheOwnCredential(deviceId: string): Promise<void> {
  const res = await fetch(`${SERVER_BASE_URL}/devices/${deviceId}/credential`);
  if (!res.ok) {
    throw new Error(`failed to fetch own credential: ${res.status}`);
  }
  const { credential } = (await res.json()) as { credential: string };
  const verified = verifyCoseSign1(base64ToBytes(credential), getServerPublicKeyBytes());
  if (!verified || bytesToUuid(decodeDeviceCredential(verified.payload).device_id) !== deviceId) {
    throw new Error('server returned a credential that failed local verification');
  }
  await cacheOwnCredential(deviceId, credential);
}
