...
/** "connected" | "disconnected" -- fired by both the peripheral and central
 * roles of the M3 payment GATT transport (BleGattTransport.kt). */
export interface BleTransportConnectionStateEvent {
  state: string;
}

/** One complete reassembled message (never a partial chunk) received over
 * the M3 payment GATT transport, from whichever role is currently active. */
export interface BleTransportDataEvent {
  data: Uint8Array;
}
...
interface TapPayNativeModuleEvents {
  ...
  onBleTransportConnectionState: (event: BleTransportConnectionStateEvent) => void;
  onBleTransportData: (event: BleTransportDataEvent) => void;
}
...
interface TapPayNativeModuleType {
  ...
  /** Starts advertising the M3 payment GATT transport (peripheral role):
   * connectable, service-data carries `ownDeviceId` so a scanning central
   * can match against the specific `recipient_device_id` it read from a
   * Request QR. */
  bleStartAdvertising(ownDeviceId: Uint8Array): Promise<void>;
  bleStopAdvertising(): Promise<void>;
  /** Scans for and connects to the peer advertising `targetDeviceId`
   * (central role), suspending until the transport is fully ready
   * (services discovered, MTU negotiated, notifications enabled) or
   * rejecting on timeout/failure. */
  bleConnectToPeer(targetDeviceId: Uint8Array, timeoutMs: number): Promise<void>;
  /** Sends `bytes` over whichever role (peripheral or central) currently has
   * an active connection. Rejects if neither does. */
  bleSend(bytes: Uint8Array): Promise<void>;
...
// --- M3 Milestone 1: the real payment GATT transport (BleGattTransport.kt) ---
// Not yet wired into PayScreen.tsx/paymentFlow.ts or any role-selection UI --
// that's Milestone 2. These are thin wrappers only, same style as every
// other function above.

export function bleStartAdvertising(ownDeviceId: Uint8Array): Promise<void> {
  return NativeModule.bleStartAdvertising(ownDeviceId);
}

export function bleStopAdvertising(): Promise<void> {
  return NativeModule.bleStopAdvertising();
}

export function bleConnectToPeer(targetDeviceId: Uint8Array, timeoutMs = 15_000): Promise<void> {
  return NativeModule.bleConnectToPeer(targetDeviceId, timeoutMs);
}

export function bleSend(bytes: Uint8Array): Promise<void> {
  return NativeModule.bleSend(bytes);
}

export function bleDisconnect(): Promise<void> {
  return NativeModule.bleDisconnect();
}

export function onBleTransportConnectionState(listener: (event: BleTransportConnectionStateEvent) => void): EventSubscription {
  return NativeModule.addListener('onBleTransportConnectionState', listener);
}

export function onBleTransportData(listener: (event: BleTransportDataEvent) => void): EventSubscription {
  return NativeModule.addListener('onBleTransportData', listener);
}
