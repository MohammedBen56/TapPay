...
/** TxRequest's field order -- see its doc comment (types.ts). Unsigned, like
 * IncomingIouInfo, so this is still a real wire contract even though nothing
 * is cryptographically bound to it. */
const TX_REQUEST_FIELD_ORDER = ["recipient_device_id", "receiver_nonce", "ts", "receiver_online"] as const;

export function encodeTxRequest(request: TxRequest): Uint8Array {
  return cborCodec.encode(TX_REQUEST_FIELD_ORDER.map((key) => request[key]));
}

/** Tolerantly accepts the legacy 3-element array (no receiver_online field --
 * the shape mobile/src/transport/qr.ts used before Phase 4) by defaulting
 * receiver_online to `false`, the conservative choice: "assume the payee
 * might not be reachable," which only ever downgrades mode selection toward
 * Mode B, never upgrades it to Mode A on a false signal. A scanner reading a
 * QR generated by an old, not-yet-rebuilt client must not crash or silently
 * treat the payee as online. */
export function decodeTxRequest(bytes: Uint8Array): TxRequest {
  const decoded = cborCodec.decode(bytes);
  if (!Array.isArray(decoded) || (decoded.length !== TX_REQUEST_FIELD_ORDER.length && decoded.length !== 3)) {
    throw new Error("malformed TxRequest CBOR: expected a 3- or 4-element array");
  }
  const [recipient_device_id, receiver_nonce, ts, receiver_online] = decoded;
  return { recipient_device_id, receiver_nonce, ts: Number(ts), receiver_online: decoded.length === 4 ? Boolean(receiver_online) : false };
}

// recipient_device_id and receiver_nonce are appended, not inserted -- field
// order is the wire contract, never reorder existing fields (see TxReceipt's
// doc comment in types.ts for why they're here at all).
const TX_RECEIPT_FIELD_ORDER = ["tx_uuid", "settled_at", "amount", "currency", "recipient_device_id", "receiver_nonce"] as const;
...
export function decodeTxReceipt(bytes: Uint8Array): TxReceipt {
  ...
  if (!Array.isArray(decoded) || decoded.length !== TX_RECEIPT_FIELD_ORDER.length) {
    throw new Error("malformed TxReceipt CBOR: expected a 6-element array");
  }
  const [tx_uuid, settled_at, amount, currency, recipient_device_id, receiver_nonce] = decoded;
  return {
    tx_uuid,
 ...
 * codec above. `own_credential` (M3 Milestone 3) is required, not
 * legacy-tolerant -- pre-release code, no deployed old-format clients. */
const SESSION_HELLO_FIELD_ORDER = ["tx_uuid", "device_id", "eph_pubkey", "ts", "own_credential"] as const;
...
export function decodeSessionHello(bytes: Uint8Array): SessionHello {
  ...
  if (!Array.isArray(decoded) || decoded.length !== SESSION_HELLO_FIELD_ORDER.length) {
    throw new Error("malformed SessionHello CBOR: expected a 5-element array");
  }
  const [tx_uuid, device_id, eph_pubkey, ts, own_credential] = decoded;
  return { tx_uuid, device_id, eph_pubkey, ts: Number(ts), own_credential };
}
