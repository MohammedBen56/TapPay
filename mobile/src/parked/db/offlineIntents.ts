  ...
  if (!dbPromise) {
    dbPromise = SQLite.openDatabaseAsync('tappay-offline.db').then(async (db) => {
      await db.execAsync(`
        ...
        CREATE TABLE IF NOT EXISTS own_credentials (
          device_id TEXT PRIMARY KEY NOT NULL,
...
/** Caches this device's OWN server-signed device credential (M3 Milestone 3),
 * base64 COSE_Sign1 bytes -- the same object GET /devices/:id/credential
 * returns, just for this device's own id, fetched opportunistically while
 * online and reused to embed into every SessionHello this device signs
 * (payments/bleTransport.ts), removing the live-network dependency the BLE
 * handshake previously had on both sides. Structurally identical to
 * cacheFreshnessToken above. */
export async function cacheOwnCredential(deviceId: string, credentialBase64: string): Promise<void> {
  const db = await openDb();
  await db.runAsync(
    'INSERT INTO own_credentials (device_id, credential) VALUES (?, ?) ON CONFLICT(device_id) DO UPDATE SET credential = ?',
    deviceId,
    credentialBase64,
    credentialBase64,
  );
...
export async function getCachedOwnCredential(deviceId: string): Promise<string | null> {
  const db = await openDb();
  const row = await db.getFirstAsync<{ credential: string }>('SELECT credential FROM own_credentials WHERE device_id = ?', deviceId);
  return row ? row.credential : null;
}
