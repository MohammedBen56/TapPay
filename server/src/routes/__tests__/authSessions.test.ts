...
describe("POST /auth/change-password", () => {
  ...
  it("revokes the session that made the change (and every other active session)", async () => {
    ...
    const refreshAfter = await app.inject({ method: "POST", url: "/auth/refresh", payload: { refresh_token: alice.refreshToken } });
...
describe("GET /auth/sessions and DELETE /auth/sessions/:id", () => {
  ...
  it("lists the caller's own active sessions, including the one just used to log in", async () => {
    ...
    const response = await app.inject({ method: "GET", url: "/auth/sessions", headers: authHeader(alice) });
  ...
  it("does not list another customer's sessions", async () => {
    ...
    const bobSessions = await app.inject({ method: "GET", url: "/auth/sessions", headers: authHeader(bob) });
  ...
  it("revokes a session by id, and its refresh token stops working", async () => {
    const alice = await createTestCustomer(app);
    const list = await app.inject({ method: "GET", url: "/auth/sessions", headers: authHeader(alice) });
    const { sessions } = list.json() as { sessions: { id: string }[] };
    const sessionId = sessions[0]!.id;

    const revoke = await app.inject({ method: "DELETE", url: `/auth/sessions/${sessionId}`, headers: authHeader(alice) });
  ...
  it("404s revoking another customer's session id", async () => {
    const [alice, bob] = await Promise.all([createTestCustomer(app), createTestCustomer(app)]);
    const list = await app.inject({ method: "GET", url: "/auth/sessions", headers: authHeader(bob) });
    const bobSessionId = (list.json() as { sessions: { id: string }[] }).sessions[0]!.id;

    const response = await app.inject({ method: "DELETE", url: `/auth/sessions/${bobSessionId}`, headers: authHeader(alice) });
  ...
  it("404s for an unknown session id", async () => {
    const alice = await createTestCustomer(app);
    const response = await app.inject({ method: "DELETE", url: `/auth/sessions/${randomUUID()}`, headers: authHeader(alice) });
    expect(response.statusCode).toBe(404);
  });

  it("requires a valid access token", async () => {
    const response = await app.inject({ method: "GET", url: "/auth/sessions" });
    expect(response.statusCode).toBe(401);
  });
});
