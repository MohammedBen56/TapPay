...
import { createIdentitySigner, enrollDevice, fetchAndCacheOwnCredential, type EnrolledIdentity } from '../crypto/identity';
  ...
  const handleSignHello = useCallback(
    async (side: Side) => {
      ...
      try {
        ...
        // M3 Milestone 3: the hello now carries our own server-signed
        // credential inline, so the PEER can verify us without a live fetch
        // of its own. fetchAndCacheOwnCredential is idempotent to call
        // repeatedly (just refreshes the cached copy).
        await fetchAndCacheOwnCredential(own.identity.deviceId);
        const cachedOwnCredential = await getCachedOwnCredential(own.identity.deviceId);
        if (!cachedOwnCredential) throw new Error('could not fetch our own device credential');
        ...
        const helloCose = await createSessionHello({
          ...
          ownCredentialCose: base64ToBytes(cachedOwnCredential),
        });
  ...
  const handlePeerHelloScanned = useCallback(
    async (side: Side, data: string) => {
      ...
      try {
        ...
        // The peer's REAL enrolled identity_pubkey -- never trust-on-first-use
        // from the pasted hello itself. M3 Milestone 3: this is now extracted
        // from the hello's OWN embedded credential and verified against the
        // pinned server key, inside deriveSessionKey -- no live fetch needed,
        // which also removes the one-phone/two-phone distinction this used to
        // need (peer.identity?.deviceId vs. the claimed device_id) for the
        // credential lookup specifically. Still tracked for display/seal-open
        // purposes below.
