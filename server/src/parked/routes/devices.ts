...
export function registerDeviceRoutes(app: FastifyInstance): void {
  app.get(
    "/devices/enroll/nonce",
    // Tighter than the global default -- this is the unauthenticated
    // memory-growth vector nonceStore.ts's own reaper/cap also guard.
    { config: { rateLimit: { max: config.rateLimitEnrollNonceMax, timeWindow: "1 minute" } } },
    async () => {
  ...
  app.post(
    "/devices/enroll",
    // Tighter than the global default -- attestation chain verification is
    // the most CPU-expensive thing this server does per request.
    { config: { rateLimit: { max: config.rateLimitEnrollMax, timeWindow: "1 minute" } } },
    async (request, reply) => {
      ...
      const outcome = await db.transaction().execute(async (trx) => {
        ...
        // above -- CLAUDE.md §5). Without this check, a re-enroll of an
        // existing device_id hits the bare INSERT below with no onConflict
        // clause, raising a raw PK violation that escapes as an untyped 500
        // with leaked Postgres text. Checking explicitly also closes a real
        // identity-takeover primitive: silently upserting identity_pubkey on
        // conflict would let anyone who learns a device_id re-point it at a
        // key they control.
        const existingDevice = await trx
          ...
          .executeTakeFirst();
        if (existingDevice) {
          const sameOwnerAndKey =
            existingDevice.user_id === account.user_id && Buffer.from(existingDevice.identity_pubkey).equals(identityPubkeyBytes);
          if (sameOwnerAndKey) {
            // Idempotent retry of an identical enrollment (e.g. a dropped
            // response after the server's own write succeeded). The stored
            // row's attestation_ok is left untouched here either way -- see
            // the response-building code below for why the CLIENT-facing
            // answer still uses this request's own fresh verification.
            return { kind: "idempotent" as const, accountId: account.account_id };
          }
        ...
        return { kind: "inserted" as const, accountId: account.account_id };
      });

      if (outcome.kind === "conflict") {
        return reply.status(409).send({ error: "DeviceAlreadyEnrolled", message: "device_id is already enrolled under a different account or key" });
      }
      if (outcome.kind === "idempotent") {
        // Mirror whichever shape the ORIGINAL enrollment actually returned --
        // an idempotent retry of a failed enrollment must still fail closed
        // (403), not silently upgrade to 200 just because the identity binding
        // matched. attestation_ok is re-derived from THIS request's own fresh
        // verification (a new nonce/chain each call), not the stored row's,
        // since the stored value only proves the identity binding is stable,
        // not that a resubmission carries valid attestation this time too.
        if (!verification.ok) {
        ...
        return reply.send({ account_id: outcome.accountId, device_id, attestation_ok: true });
      }
